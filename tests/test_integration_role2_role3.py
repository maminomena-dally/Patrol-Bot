...
def follow_path_localized(robot, localizer, odom, detector, controller,
                          path, max_time=120.0, est_history=None):
    ...
    Si est_history est une liste, y ajoute {"x", "y", "theta", "t"} à chaque pas.
    """
    dt = config.DT
    ...
    for step_i in range(max_steps):
        ...
        # Collecter la pose estimée pour visualisation
        if est_history is not None:
            est_history.append({
                "x": est.x, "y": est.y, "theta": est.theta,
                "t": round(robot.time, 3),
            })
...
def draw_warehouse(ax, obstacles, title="", patches=None):
...
def plot_result(robot_history, obstacles, waypoints, landmarks,
    ...
    draw_warehouse(ax, obstacles, title, patches=patches)
    ...
    if est_history and len(est_history) > 0:
        ...
        eys = [s["y"] for s in est_history]
        ax.plot(exs, eys, "--", color="#E53935", linewidth=1.4,
                label="Trajectoire EKF (estimée)", alpha=0.8, zorder=3)

    # Trajectoire RÉELLE du robot (ligne bleue)
    if robot_history:
        ...
        if est_history and len(est_history) > 0:
            step_conn = max(1, len(robot_history) // 40)
            for i in range(0, min(len(robot_history), len(est_history)), step_conn):
                ax.plot(
                    [robot_history[i]["x"], est_history[i]["x"]],
                    [robot_history[i]["y"], est_history[i]["y"]],
                    "-", color="#9E9E9E", linewidth=0.5, alpha=0.5, zorder=2,
                )
...
def plot_layout_only(save_path):
    ...
    ensure_dir(save_path)
    fig, ax = plt.subplots(figsize=(14, 10.5))
    draw_warehouse(ax, WAREHOUSE_OBSTACLES, "Plan d'entrepôt — Layout", patches=patches)
...
def plot_localization_error(robot_history, est_history, title, save_path):
    """Génère un graphique de l'erreur de localisation au fil du temps.

    Deux subplots :
      - Haut : erreur de position (m) vs temps
      - Bas : erreur de cap theta (rad) vs temps
    """
    ...
    if not robot_history or not est_history:
        return

    n = min(len(robot_history), len(est_history))
    times = [est_history[i]["t"] for i in range(n)]
    pos_err = [math.hypot(
        robot_history[i]["x"] - est_history[i]["x"],
        robot_history[i]["y"] - est_history[i]["y"],
    ) for i in range(n)]
    theta_err = [abs(normalize_angle(
        robot_history[i].get("theta", 0) - est_history[i]["theta"]
    )) for i in range(n)]
    ...
    # --- Erreur de position ---
    ax1.plot(times, pos_err, "-", color="#1565C0", linewidth=0.8, alpha=0.8)
    ax1.fill_between(times, 0, pos_err, alpha=0.15, color="#1565C0")
    ax1.set_ylabel("Erreur position (m)")
    ax1.grid(True, alpha=0.3)
    ax1.axhline(y=0.5, color="#F44336", linestyle="--", alpha=0.5, label="Seuil safety (0.5m)")
    max_pe = max(pos_err) if pos_err else 0
    mean_pe = sum(pos_err) / len(pos_err) if pos_err else 0
    ax1.legend(fontsize=9)
    ax1.text(0.98, 0.95, f"max={max_pe:.3f}m\nmoy={mean_pe:.3f}m",
             transform=ax1.transAxes, ha="right", va="top",
             fontsize=9, bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.8))

    # --- Erreur de cap ---
    ax2.plot(times, [math.degrees(e) for e in theta_err], "-", color="#E53935", linewidth=0.8, alpha=0.8)
    ax2.fill_between(times, 0, [math.degrees(e) for e in theta_err], alpha=0.15, color="#E53935")
    ax2.set_ylabel("Erreur cap (deg)")
    ax2.set_xlabel("Temps (s)")
    ax2.grid(True, alpha=0.3)
    max_te = max(theta_err) if theta_err else 0
    mean_te = sum(theta_err) / len(theta_err) if theta_err else 0
    ax2.text(0.98, 0.95, f"max={math.degrees(max_te):.1f}deg\nmoy={math.degrees(mean_te):.1f}deg",
...
def export_ekf_csv(robot_history, est_history, filepath):
    """Exporte un CSV comparatif true vs estimé."""
    ensure_dir(filepath)
    n = min(len(robot_history), len(est_history))
    if n == 0:
        return
    with open(filepath, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["t", "true_x", "true_y", "true_theta",
                         "est_x", "est_y", "est_theta",
                         "err_pos_m", "err_theta_deg"])
        for i in range(n):
            rh = robot_history[i]
            eh = est_history[i]
            err_p = math.hypot(rh["x"] - eh["x"], rh["y"] - eh["y"])
            err_t = abs(normalize_angle(rh.get("theta", 0) - eh["theta"]))
            writer.writerow([
                f'{eh["t"]:.3f}',
                f'{rh["x"]:.4f}', f'{rh["y"]:.4f}',
                f'{rh.get("theta", 0):.4f}',
                f'{eh["x"]:.4f}', f'{eh["y"]:.4f}', f'{eh["theta"]:.4f}',
                f'{err_p:.4f}', f'{math.degrees(err_t):.2f}',
            ])
...
def main():
    ...
    plot_localization_error(
        robot_astar.history, est_hist_astar,
        "A* — Erreur de localisation EKF vs Vraie pose",
        os.path.join(RESULTS_DIR, "localization_error_astar.png"),
    )
    export_ekf_csv(
        robot_astar.history, est_hist_astar,
        os.path.join(RESULTS_DIR, "ekf_comparison_astar.csv"),
    )
